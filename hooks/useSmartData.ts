...
export const useSmartSystemData = (currentUser: UserPermission | null) => {
    ...
    const refreshSystemData = useCallback(async (force = false) => {
        ...
        try {
            ...
            // Chỉ lấy News và InvoiceSettings (Công khai)
            ...
            if (!isAdmin) {
                // LUỒNG CƯ DÂN: Chỉ fetch dữ liệu liên quan trực tiếp đến residentId
                ...
                // LUỒNG ADMIN: Fetch toàn bộ bảng nghiệp vụ
                ...
                const [charges, water, stats, locks] = await Promise.all([
