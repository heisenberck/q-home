            ...
            <div className="flex items-center justify-between mb-6 bg-white p-4 rounded-xl shadow-sm border border-gray-200">
                <div>
                    ...
                    <p className="text-sm text-gray-500 mt-1">
                        Tìm thấy <strong className="text-red-600">{groups.length}</strong> nhóm biển số trùng nhau.
                    </p>
                </div>
                    ...
                    <div className="grid grid-cols-2 gap-3 mb-3">
                        {["Cập nhật biển số", "Cập nhật loại xe", "Cập nhật hình ảnh", "Cập nhật lốt xe"].map(r => (
                            <label key={r} className="flex items-center gap-2 cursor-pointer hover:bg-gray-50 p-1 rounded -ml-1 select-none">
                                <input type="checkbox" checked={selectedReasons.includes(r)} onChange={() => toggleReason(r)} className="w-4 h-4 text-primary rounded border-gray-300 focus:ring-primary" />
                                <span className="text-sm text-gray-700">{r}</span>
                            </label>
                        ))}
                    </div>
                    <input type="text" value={otherReason} onChange={e => setOtherReason(e.target.value)} className={inputClass} placeholder="Chi tiết khác..." />
...
const VehicleDetailPanel: React.FC<{
    ...
    const relevantLogs = useMemo(() => {
        return activityLogs.filter(log => (log.ids && log.ids.includes(vehicle.VehicleId)) || log.summary.includes(vehicle.PlateNumber)).slice(0, 10);
            ...
            <div className="p-6 space-y-8 flex-1">
                <section>
                    ...
                    <div className="bg-gray-50 rounded-xl p-4 border border-gray-100">
                        <div className="flex justify-between mb-2"><span className="text-sm text-gray-500">Chủ hộ</span><span className="text-sm font-bold text-gray-900">{vehicle.ownerName}</span></div>
                        <div className="flex justify-between mb-2"><span className="text-sm text-gray-500">Căn hộ</span><span className="text-sm font-bold bg-white border px-2 rounded">{vehicle.UnitID}</span></div>
                        <div className="flex justify-between"><span className="text-sm text-gray-500">Liên hệ</span><a href={`tel:${vehicle.ownerPhone}`} className="text-sm font-bold text-blue-600 hover:underline flex items-center gap-1"><PhoneArrowUpRightIcon className="w-3 h-3"/> {vehicle.ownerPhone}</a></div>
                    </div>
                </section>
                <section>
                    ...
                    <div className="space-y-3">
                        <div className="flex justify-between text-sm border-b border-gray-100 pb-2"><span className="text-gray-500">Ngày đăng ký</span><span className="font-medium">{new Date(vehicle.StartDate).toLocaleDateString('vi-VN')}</span></div>
                        <div className="flex justify-between text-sm border-b border-gray-100 pb-2"><span className="text-gray-500">Trạng thái phí</span>{vehicle.isBillable ? <span className="text-green-600 font-bold text-xs bg-green-50 px-2 py-0.5 rounded">Được tính phí</span> : <span className="text-gray-500 font-bold text-xs bg-gray-100 px-2 py-0.5 rounded">Miễn phí / Chờ</span>}</div>
                    </div>
                </section>
                <section>
                    ...
                    <div className="grid grid-cols-2 gap-4">
                        {['registration', 'vehiclePhoto'].map(type => (
                             <div key={type}>
                                <p className="text-xs font-medium text-gray-500 mb-1.5">{type === 'registration' ? 'Đăng ký xe' : 'Ảnh xe'}</p>
                                {vehicle.documents?.[type as 'registration'|'vehiclePhoto']?.url ? (
                                    <div onClick={() => onOpenDoc(vehicle.documents![type as 'registration'|'vehiclePhoto']!)} className="block h-24 w-full rounded-lg overflow-hidden border border-gray-200 relative group cursor-pointer">
                                        <img src={vehicle.documents![type as 'registration'|'vehiclePhoto']!.url} className="w-full h-full object-cover transition-transform duration-500 group-hover:scale-110" alt={type} />
                                        <div className="absolute inset-0 bg-black/0 group-hover:bg-black/10 transition-colors flex items-center justify-center"><EyeIcon className="w-6 h-6 text-white opacity-0 group-hover:opacity-100 drop-shadow-md" /></div>
                                    </div>
                                ) : (
                                    <div className="h-24 w-full rounded-lg border-2 border-dashed border-gray-200 bg-gray-50 flex items-center justify-center text-gray-400 text-xs">Chưa có ảnh</div>
                                )}
                            </div>
                        ))}
                    </div>
...
const VehiclesPage: React.FC<VehiclesPageProps> = ({ vehicles, units, owners, activityLogs, onSetVehicles, role }) => {
    ...
    const filteredVehicles = useMemo(() => {
        return enhancedVehicles.filter(v => {
            ...
            const pa = parseUnitCode(a.UnitID); const pb = parseUnitCode(b.UnitID);
    ...
    useEffect(() => { setCurrentPage(1); }, [searchTerm, typeFilter, statusFilter, kpiFilter]);
    ...
    const handleSave = async (updatedVehicle: Vehicle, reason: string) => {
        ...
        try {
            ...
            showToast('Lỗi lưu dữ liệu: ' + (error.message || "Unknown error"), 'error');
        } finally {
            setIsLoading(false);
        }
    };
    ...
    const handleBatchDelete = async (idsToDelete: string[]) => {
        ...
        try {
            ...
            showToast('Lỗi xóa trùng lặp: ' + error.message, 'error');
        } finally {
            setIsLoading(false);
        }
    };
    ...
    const handleExport = () => {
        ...
        try {
            const dataToExport = filteredVehicles.map(v => ({
                'Căn hộ': v.UnitID,
                'Chủ hộ': v.ownerName,
                'Số điện thoại': v.ownerPhone,
                'Biển số': v.PlateNumber,
                'Loại xe': translateVehicleType(v.Type as VehicleTier),
                'Tên xe': v.VehicleName || '',
                'Trạng thái đỗ': v.parkingStatus || 'Chưa gán',
        ...
        <div className="flex gap-6 h-full overflow-hidden relative">
            ...
            {isDuplicateMode ? (
                <div className="w-full h-full">
                    <DuplicateManager vehicles={enhancedVehicles.filter(v => v.isActive)} onClose={() => setIsDuplicateMode(false)} onDeleteBatch={handleBatchDelete} />
                </div>
            ) : (
                <>
                    ...
                    {selectedVehicle && (<div className="w-1/3 flex flex-col h-full animate-slide-up shadow-2xl rounded-l-xl overflow-hidden z-20"><VehicleDetailPanel vehicle={selectedVehicle} activityLogs={activityLogs} onEdit={(v) => setEditingVehicle(v)} onDelete={() => handleDelete(selectedVehicle)} onClose={() => setSelectedVehicle(null)} onOpenDoc={setPreviewDoc} /></div>)}
                </>
            )}
        </div>
    );
