    ...
    const handleRegSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            const content = regType === 'construction'
                ? `Đơn vị: ${regData.contractor}. Thời gian: ${regData.constructionTime}`
            ...
            setRegType(null);
            setRegData({ plate: '', model: '', color: '', constructionItem: '', constructionTime: '', contractor: '' });
        } catch {
                        ...
                        {!regType ? (
                            ...
                            <form onSubmit={handleRegSubmit} className="space-y-5">
                                ...
                                {regType === 'construction' ? (
                                    <>
                                        <div><label className={labelStyle}>Hạng mục thi công</label><input value={regData.constructionItem} onChange={e => setRegData({...regData, constructionItem: e.target.value})} className={inputStyle} placeholder="VD: Sơn lại tường, Lát sàn gỗ..." required /></div>
                                        <div><label className={labelStyle}>Thời gian dự kiến</label><input value={regData.constructionTime} onChange={e => setRegData({...regData, constructionTime: e.target.value})} className={inputStyle} placeholder="Từ ngày ... đến ngày ..." required /></div>
                                        <div><label className={labelStyle}>Đơn vị thực hiện</label><input value={regData.contractor} onChange={e => setRegData({...regData, contractor: e.target.value})} className={inputStyle} placeholder="Tên công ty hoặc tổ thợ..." required /></div>
                                    </>
                                ) : (
                                    <>
                                ...
                                <button type="submit" disabled={isSubmitting} className="w-full py-4 bg-emerald-600 text-white font-black uppercase rounded-2xl shadow-xl shadow-emerald-100 transition-all flex items-center justify-center gap-2">
