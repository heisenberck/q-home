...
const PortalContactPage: React.FC<PortalContactPageProps> = ({ hotline, onSubmitFeedback, owner, unit }) => {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            const newFeedback: Omit<FeedbackItem, 'id'> = {
            ...
            showToast('Gửi thành công! Ban Quản lý đã nhận được thông báo.', 'success');
    ...
    <div className="p-4 space-y-6 pb-24">
        ...
        <div className="bg-white p-5 rounded-2xl shadow-sm border border-gray-100">
            ...
            <form onSubmit={handleSubmit} className="space-y-5">
                            ...
                            <div key={slot.id} className={`relative flex flex-col items-center justify-center p-4 border-2 border-dashed rounded-2xl transition-all ${slot.file ? 'bg-emerald-50 border-emerald-300' : 'bg-gray-50 border-gray-200 hover:bg-gray-100'}`}>
                                {slot.file ? (
                                    <>
                                        ...
                                        <button type="button" onClick={() => handleFileInSlot(slot.id, null)} className="absolute top-2 right-2 p-1 bg-white rounded-full shadow-sm text-red-500"><XMarkIcon className="w-4 h-4" /></button>
                                    </>
                                ) : (
                                    <label className="cursor-pointer flex flex-col items-center justify-center w-full h-full">
                                        ...
                                        <input type="file" accept="image/*" className="hidden" onChange={e => handleFileInSlot(slot.id, e.target.files?.[0] || null)} />
                                    </label>
                                )}
                            </div>
                        ))}
                ...
                >
                    {isSubmitting ? <Spinner /> : <><DocumentTextIcon className="w-5 h-5" /> Gửi yêu cầu ngay</>}
                </button>
            </form>
        </div>
    </div>
