...
const ValueAddedServicesPage: React.FC = () => {
    ...
    const handleSyncToSheets = async (item: MiscRevenue) => {
        ...
        try {
            ...
            formData.append('action_type', 'SYNC_VAS');
            formData.append('id', item.id);
            formData.append('date', item.date);
            formData.append('type', typeLabels[item.type]);
            ...
            if (response.ok) {
                showToast(`Đã đồng bộ "${item.description}" sang Google Sheets.`, 'success');
            } else {
    ...
    const handleExport = () => {
        ...
        try {
            ...
            XLSX.utils.book_append_sheet(workbook, worksheet, "Doanh thu VAS");
            XLSX.writeFile(workbook, `Bao_cao_VAS_Thang_${monthLabel}.xlsx`);
                                ...
                                <table className="w-full text-sm border-collapse">
                                    <thead className="sticky top-0 z-10 bg-white/95 backdrop-blur-md shadow-sm">
                                        <tr className="text-left text-[9px] font-black text-gray-400 uppercase tracking-widest border-b border-gray-50">
                                            <th className="px-6 py-3.5 w-24">Giờ nộp</th>
                                            ...
                                            <th className="px-6 py-3.5 w-24 text-center">Công cụ</th>
                                        </tr>
                                    </thead>
