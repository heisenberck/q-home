...
import { 
    ...
    SaveIcon, LockClosedIcon, XMarkIcon, ArrowPathIcon, TrashIcon
} from '../ui/Icons';
...
const WaterPage: React.FC<WaterPageProps> = ({ waterReadings: globalReadings, setWaterReadings, allUnits, role, tariffs, lockedPeriods = [], refreshData }) => {
    ...
    const waterReadingsMap = useMemo(() => {
        ...
        localReadings.forEach(r => { if (!map.has(r.UnitID)) map.set(r.UnitID, []); map.get(r.UnitID)!.push(r); });
        return map;
    }, [localReadings]);

    const waterData = useMemo(() => {
        const prevReadingPeriod = getPreviousPeriod(period);
        const prevPeriodReadings = new Map<string, WaterReading>(localReadings.filter(r => r.Period === prevReadingPeriod).map(r => [r.UnitID, r]));
        const currentPeriodReadings = new Map<string, WaterReading>(localReadings.filter(r => r.Period === period).map(r => [r.UnitID, r]));
    ...
    const historicalChartData = useMemo(() => {
        ...
        for (let i = 0; i < 6; i++) {
            ...
            let consumption = localReadings.filter(r => r.Period === p).reduce((total, reading) => total + (reading.consumption ?? 0), 0);
        ...
        return data.reverse();
    }, [period, localReadings]);

    const analyticsData = useMemo(() => {
        const prevPeriod = getPreviousPeriod(period);
        const prevPeriodConsumptionMap = new Map<string, number>();
        localReadings.filter(r => r.Period === prevPeriod).forEach(r => { prevPeriodConsumptionMap.set(r.UnitID, r.consumption ?? 0); });
    ...
    }, [waterData, period, localReadings]);

    const individualUnitAnalytics = useMemo(() => {
        if (!selectedUnitId) return null;
        const uniqueReadings = localReadings.filter(r => r.UnitID === selectedUnitId && r.consumption > 0).sort((a, b) => b.Period.localeCompare(a.Period)).slice(0, 12);
        if (uniqueReadings.length === 0) return { highest: null, lowest: null, average: 0 };
        const highest = uniqueReadings.reduce((max, current) => current.consumption > max.consumption ? current : max, uniqueReadings[0]);
        const lowest = uniqueReadings.reduce((min, current) => current.consumption < min.consumption ? current : min, uniqueReadings[0]);
        const total = uniqueReadings.reduce((sum, current) => sum + current.consumption, 0);
        const average = total / uniqueReadings.length;
        return { highest, lowest, average };
    }, [selectedUnitId, localReadings]);
    ...
    const handleResetAll = async () => {
        ...
        if (!window.confirm(`Bạn có chắc chắn muốn XÓA TẤT CẢ chỉ số đã nhập trong kỳ ${period}? Hành động này không thể hoàn tác.`)) return;
        
        setIsFetching(true);
        try {
            const currentReadings = localReadings.filter(r => r.Period === period);
            const resetReadings = currentReadings.map(r => ({ ...r, CurrIndex: r.PrevIndex, consumption: 0 }));
            // To properly reset, we should delete them from DB or save them as empty/same as prev
            // Based on handleSave logic, sending empty string/null deletes them.
            // Let's delete them for this period.
            const readingsToKeep = localReadings.filter(r => r.Period !== period);
            await saveWaterReadings(readingsToKeep); // Re-save without this period
            setLocalReadings(readingsToKeep);
            if (refreshData) refreshData(true);
            showToast(`Đã xóa sạch chỉ số nước kỳ ${period}.`, 'success');
        } catch (error) {
    ...
    return (
        <div className="flex gap-6 h-full overflow-hidden">
            ...
            {isImportModalOpen && <ImportModal onClose={() => setIsImportModalOpen(false)} onDownloadTemplate={() => {}} onTriggerUpload={() => {}} />}
                ...
                <div className="bg-white p-4 rounded-xl shadow-sm flex-shrink-0">
                     <div className="flex items-center gap-2 md:gap-4">
                         ...
                         <div className="flex items-center gap-2 border-l pl-4">
                            ...
                            {canEdit && !isLocked && (
                                <button onClick={handleResetAll} className="h-10 px-4 font-semibold rounded-lg flex items-center gap-2 border border-red-200 text-red-600 hover:bg-red-50 bg-white" title="Xóa toàn bộ chỉ số kỳ này"><TrashIcon className="w-5 h-5" /> Reset</button>
                            )}
                            {isLocked ? (
                                <button className="h-10 px-4 font-semibold rounded-lg flex items-center gap-2 bg-gray-400 text-white cursor-not-allowed"><LockClosedIcon className="w-5 h-5" /> Đã chốt</button>
                            ) : (
                                <button onClick={() => {}} className="h-10 px-4 font-semibold rounded-lg flex items-center gap-2 bg-primary text-white hover:bg-primary-focus"><SaveIcon className="w-5 h-5" /> Chốt sổ</button>
                            )}
