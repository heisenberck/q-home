...
const DataImportModal: React.FC<{
    ...
    const processExcelData = () => {
        ...
        const updates = preview.map(rowArray => {
            ...
            return { unitId, unitType, ownerName: String(row.ownerName || ''), status: row.status || 'Owner', area: parseFloat(String(row.area||'0'))||0, phone: normalizePhoneNumber(row.phone||''), email: String(row.email||''), vehicles, parkingStatus: row.parkingStatus||null, };
        }).filter(Boolean);
        onImport(updates);
        onClose();
    };
