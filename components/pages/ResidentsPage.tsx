    ...
    return (
        <div className="flex flex-col h-full bg-white border-l border-gray-200 shadow-2xl overflow-y-auto animate-slide-up">
            ...
            <div className="border-b border-gray-100">
                <button onClick={() => setOwnerInfoOpen(!isOwnerInfoOpen)} className="w-full flex items-center justify-between p-4 bg-gray-50/50 hover:bg-gray-100 transition-colors">
                    <span className="text-sm font-bold text-gray-700 uppercase flex items-center gap-2"><UserIcon className="w-4 h-4"/> Thông tin chủ hộ</span>
                    {isOwnerInfoOpen ? <ChevronUpIcon className="w-4 h-4 text-gray-500"/> : <ChevronDownIcon className="w-4 h-4 text-gray-500"/>}
                </button>
                {isOwnerInfoOpen && (
                    <div className="p-4 space-y-4 animate-fade-in-down">
                        <div className="grid grid-cols-2 gap-4">
                            <div><label className="text-[10px] text-gray-500 font-semibold uppercase tracking-wider block mb-1">Chủ hộ</label><div className="text-sm font-semibold text-gray-800">{owner.OwnerName}</div></div>
                            <div><label className="text-[10px] text-gray-500 font-semibold uppercase tracking-wider block mb-1">SĐT</label><a href={`tel:${owner.Phone}`} className="text-sm font-semibold text-blue-600 hover:underline">{owner.Phone}</a></div>
                        </div>
                        ...
                        <div className="pt-3 border-t border-dashed border-gray-200 grid grid-cols-2 gap-4">
                            <div><label className="text-[10px] text-gray-500 font-semibold uppercase tracking-wider block mb-1">Vợ/Chồng/Khách thuê</label><div className="text-sm font-semibold text-gray-800">{owner.secondOwnerName || '---'}</div></div>
                            <div><label className="text-[10px] text-gray-500 font-semibold uppercase tracking-wider block mb-1">SĐT liên hệ 2</label><div className="text-sm font-semibold text-gray-800">{owner.secondOwnerPhone || '---'}</div></div>
                        </div>
                    </div>
                )}
            </div>
            ...
            <div className="border-b border-gray-100">
                <button onClick={() => setVehiclesOpen(!isVehiclesOpen)} className="w-full flex items-center justify-between p-4 bg-gray-50/50 hover:bg-gray-100 transition-colors">
                    <span className="text-sm font-bold text-gray-700 uppercase flex items-center gap-2"><CarIcon className="w-4 h-4"/> Phương tiện <span className="text-xs bg-gray-200 px-1.5 rounded-full text-gray-600">{safeVehicles.filter(v=>v?.isActive).length}</span></span>
                ...
                {isVehiclesOpen && (
                    <div className="p-4 space-y-2 animate-fade-in-down">
                        {safeVehicles.filter(v=>v?.isActive).length > 0 ? (
                            ...
                            <div className="text-center py-4 text-gray-400 text-sm italic bg-gray-50 rounded-lg border border-dashed border-gray-200">Chưa đăng ký phương tiện.</div>
                        )}
                    </div>
                )}
            </div>
            ...
            <div className="p-6 space-y-8 flex-1">
                ...
                <div className="mt-auto pt-4 border-t">
                    <button onClick={() => onExportPDF(resident)} className="w-full py-2.5 bg-white border border-gray-300 text-gray-700 font-semibold rounded-lg shadow-sm hover:bg-gray-50 flex items-center justify-center gap-2 transition-colors">
...
const ResidentsPage: React.FC<ResidentsPageProps> = ({ units = [], owners = [], vehicles = [], activityLogs = [], onSaveResident, onImportData, onDeleteResidents, role, currentUser, onNavigate }) => {
    ...
    useEffect(() => {
        window.addEventListener('REFRESH_RESIDENTS', handleRefreshEvent);
        return () => window.removeEventListener('REFRESH_RESIDENTS', handleRefreshEvent);
    }, [handleRefreshEvent]);
                ...
                <div className="bg-white p-4 rounded-xl shadow-sm">
                    <div className="flex items-center gap-2 md:gap-4">
                        ...
                        <div className="ml-auto flex items-center gap-2">
                            ...
                            <button onClick={handleExportExcel} className="h-10 px-4 font-semibold rounded-lg flex items-center gap-2 border border-green-600 text-green-700 hover:bg-green-600/10 bg-white"><DocumentArrowDownIcon /> Export</button>
                            <button onClick={() => handleRefreshEvent()} disabled={isLoading} className="h-10 w-10 flex items-center justify-center rounded-lg border border-gray-200 bg-white text-gray-500 hover:text-primary hover:border-primary transition-all disabled:opacity-30">
